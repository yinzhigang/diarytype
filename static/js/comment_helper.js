...
function save_cmt_cookie(name, email, homepage)
{
	var now = new Date();
	var expires = new Date(now.getTime()+3600000*24*30);
	set_cookie('cmt_name', name, expires, '/');
	set_cookie('cmt_email', email, expires, '/');
	set_cookie('cmt_homepage', homepage, expires, '/');
}
...
function clear_cmt_cookie()
{
	...
	set_cookie('cmt_name', 0, expires, '/');
	set_cookie('cmt_email', 0, expires, '/');
	set_cookie('cmt_homepage', 0, expires, '/');
	set_cookie('cmt_remember', 'no', new Date(now.getTime()+3600000*24*1000), '/');
}
